...
# =============== دالة الحصول على الرابط العام ===============
def get_public_base_url():
    """الحصول على الرابط العام للمنصة (يعمل للجميع)"""
    
    # 1. منصات السحابة (Railway, Render, Heroku)
    ...
    # 2. Ngrok (إذا كان موجود ومشغل)
    try:
        ngrok_tunnels = requests.get("http://localhost:4040/api/tunnels", timeout=2).json()
        if ngrok_tunnels.get("tunnels"):
            return ngrok_tunnels["tunnels"][0]["public_url"]
    except:
        pass
    
    # 3. IP العام الحقيقي (عبر خدمة خارجية)
    try:
        public_ip = requests.get("https://api.ipify.org", timeout=5).text
        port = os.environ.get("PORT", 3034)
        return f"http://{public_ip}:{port}"
    except:
        pass
    
    # 4. IP المحلي (لنفس الشبكة فقط - أخير حل)
    try:
...
def get_server_public_url(owner: str, folder: str, port: int = None) -> str:
    """الحصول على الرابط العام لمشروع معين"""
    meta = read_meta(owner, folder)
    project_port = port or meta.get("port", 5000)
    ...
    # أخيراً: استخدام hostname المحلي
    try:
        hostname = socket.gethostname()
        return f"http://{hostname}:{project_port}"
    except:
        return f"http://localhost:{project_port}"
...
def read_meta(owner: str, folder: str):
    ...
    try:
        ...
        return {"display_name": folder, "startup_file": "", "owner": owner, "banned": False, "port": 5000, "custom_domain": ""}
...
def start_project(owner: str, folder: str, startup_file: str):
    ...
    if is_flask:
        ...
        log_append(f"{owner}::{folder}", f"[SYSTEM] Access at: {get_server_public_url(owner, folder, port)}\n")
...
def server_stats(key):
    ...
    if meta.get("banned", False):
        ...
        set_state(key, "Offline")
    port = meta.get("port", 5000)
    url = get_server_public_url(owner, folder, port) if state == "Running" else ""
    return jsonify({
        "status": state,
        "cpu": cpu,
        "mem": mem,
